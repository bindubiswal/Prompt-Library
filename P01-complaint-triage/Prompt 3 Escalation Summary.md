...
- Original complaint text: [COMPLAINT_TEXT]
- Customer contact details: [CUSTOMER_CONTACT]
- Date received: [DATE_RECEIVED]
- Assigned agent: [AGENT_NAME]

Your escalation summary must include these sections:
...
After completing all sections, review your own summary and check:
- Have you included any information not provided to you? If yes, remove it.
- Is the risk assessment based on evidence in the complaint, not assumption? If not, revise it.

Format: Plain structured text. No bullet points within sections. Maximum 200 words total.
```
...
|-------------|--------|---------|
| `[CATEGORY]` | P01 output | "Staff conduct" |
| `[URGENCY_REASON]` | P01 one_line_reason field | "Complaint involves alleged discrimination by staff member" |
| `[COMPLAINT_TEXT]` | Original complaint | "The staff member at your Doncaster store made a racially discriminatory comment..." |
| `[CUSTOMER_CONTACT]` | CRM record | "customer@email.com / 0400 000 000" |
| `[DATE_RECEIVED]` | CRM timestamp | "11 April 2025, 9:32am" |
| `[AGENT_NAME]` | CRM ticket assignment | "Sarah M." |

---

## 🏢 Intended Workflow or Task

- **Trigger:** P01 assigns urgency = High; manager alert is triggered automatically
- **Actor:** Customer Service Manager receives this summary and acts within 2 hours
- **Timing:** Generated within 60 seconds of High-urgency classification
- **Next step:** Manager reviews summary, contacts customer directly or escalates to legal/HR team

```
P01 output: urgency = High
        │
        ▼
[P03 RUNS] → Escalation summary generated
        │
        ▼
Manager receives alert + summary (target: within 60 seconds of complaint receipt)
        │
        ├─→ Legal risk → Refer to legal team
        ├─→ Safety risk → Incident report + store contact
        ├─→ Reputational risk → Senior management notification
        └─→ Financial risk → Finance team + customer callback authorised
```
...
- Delayed escalation of high-risk complaints (average 45–90 min under manual process)
- Inconsistent information provided to managers (some agents include irrelevant detail; others omit critical facts)
- No structured risk categorisation to guide manager's immediate response
- Legal risk from unmanaged complaints reaching media or legal representation before internal response
...
**Test date:** 10 April 2025 | **Sample:** 15 High-urgency complaint escalations | **Evaluator:** Customer Service Manager

| Criteria | v1.0 | v1.1 |
|----------|------|------|
| Zero fabricated details | 40% | 100% |
| Correct risk category assigned | 60% | 93% |
| Manager confidence in summary (1–5) | 2.1/5 | 4.7/5 |
| Time to manager action | 45–90 min | <5 min |
