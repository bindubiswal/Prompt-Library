# Prompt1 · Complaint Triage and Classification
...
- **Next step:** Prompt2 (Customer response draft) is queued for the assigned team agent
...
- Routing inconsistencies that are causing uneven team workloads
...
|-----------|------------|
| Repetitiveness | Extremely high as in identical classification task performed on every complaint |
| Data availability | Input is the complaint text itself because it was always available at point of receipt |
...
| Urgency misclassified as serious complaint was marked Low | High | Explicit urgency criteria in prompt ("personal injury, discrimination, legal action"); manager reviews all High flags within 1 hour |
...
| Bias was played as complaint tone affecting urgency rating | Medium | Urgency criteria are explicit and criteria-based (injury, legal, financial loss), not tone-based; weekly audit monitors for patterns |
...
**Date:** 5 April 2025
**Change:** Added JSON output requirement for categories still undefined as model chose its own
**Output:** JSON returned but categories varied: "shipping", "logistics", "late delivery" all used for same complaint type
**Observed effect:** CRM routing broke as categories did not match routing rules. Tickets fell into unrecognized routed queue.
...
**Date:** 10 April 2025
**Change:** Added explicit fixed category list; added specific urgency criteria; added "Other" catch-all; added one line reason field for audit trail
...
**Lesson learned:** Constrained output categories + explicit criteria = reliable enough for automation. "Other" is essential as it never forces the model into a category that doesn't fit.
